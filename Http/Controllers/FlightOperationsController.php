...
class FlightOperationsController extends Controller
{
    ...
    public function deleteSimbrief(Request $request, $id)
    {
        $simbrief = SimBrief::findOrFail($id);
        try {
            $simbrief->delete();

            if (!$simbrief->flight_id) {
                return redirect()->route('frontend.flights.index')
                    ->with('success', 'SimBrief successfully deleted!');
            }

            return redirect()->route('frontend.simbrief.generate', ['flight_id' => $simbrief->flight_id, 'aircraft_id' => $simbrief->aircraft_id])
                ->with('success', 'SimBrief successfully deleted!');
        } catch (\Exception $e) {
            Log::error('Error deleting SimBrief: ' . $e->getMessage());
            Log::error('Stack trace: ' . $e->getTraceAsString());

            return redirect()->back()
                ->with('error', 'Error deleting SimBrief: ' . $e->getMessage())
                ->withInput();
        }
    }
}
